...
class Twitter(object):
    ...
    def __consumer_key(self):
        return Konto.get('dummy').oauth_token

    def __consumer_secret(self):
        return Konto.get('dummy').oauth_secret

    def lag_twitter_autentiserings_url(self):
        return oauth.TwitterClient(self.__consumer_key(), self.__consumer_secret(), self.callback_url).get_authorization_url()
    
    def lagre_auth_data(self, request, bidrager):
        client = oauth.TwitterClient(self.__consumer_key(), self.__consumer_secret(), self.callback_url)
